...
export async function alignMFCC_DTW(mfccFrames1, mfccFrames2, windowLength, distanceFunctionKind = 'euclidian', centerIndexes) {
    if (distanceFunctionKind == 'euclidian') {
        ...
        const { path } = alignDTWWindowed(indexes1, indexes2, (i, j) => cosineDistancePrecomputedMagnitudes(mfccFrames1[i], mfccFrames2[j], magnitudes1[i], magnitudes2[j]), windowLength, centerIndexes);
        return path;
    }
    else {
