...
import { encodeRawAudioToWave, fadeAudioInOut, getRawAudioDuration, sliceRawAudioByTime } from './AudioUtilities.js';
...
import { addWordTextOffsetsToTimeline } from '../utilities/Timeline.js';
...
export async function playAudioFileWithTimelineFile(audioFilename, timelineFileName, transcriptFileName) {
...
export async function playAudioWithWordTimeline(rawAudio, wordTimeline, transcript) {
    if (!transcript) {
    ...
    function onTimePosition(timePosition) {
    ...
    function keypressHandler(message) {
...
export async function playAudioWithTimelinePhones(rawAudio, timeline) {
    let wordIndex = 0;
    let phoneIndex = 0;
    function onTimePosition(timePosition) {
...
export async function playAudioPairWithTimelineInterleaved(rawAudio1, rawAudio2, timeline1, timeline2) {
...
export function playAudioSamples(rawAudio, onTimePosition, signalChannel, microFadeInOut = true) {
    return new Promise(async (resolve, reject) => {
        ...
        const playerSpawnedOpenPromise = new OpenPromise();
        ...
        if (streamToStdin) {
            ...
            playerProcess = spawn(soxPath, ['-t', 'raw', '-r', `${rawAudio.sampleRate}`, '-e', 'signed', '-b', '16', '-c', channelCount.toString(), '-', '-d'], {});
        }
        else {
            ...
            playerProcess = spawn(soxPath, [tempFilePath, '-d'], {});
        }
        if (signalChannel) {
...
export function playAudioSamples_Speaker(rawAudio, onTimePosition, microFadeInOut = true) {
    return new Promise(async (resolve, reject) => {
        if (microFadeInOut) {
            rawAudio = fadeAudioInOut(rawAudio, 0.0025);
        }
        ...
        const byteCountToDuration = (byteCount) => {
            return byteCount / bytesPerSecond;
        };
        const audioDuration = byteCountToDuration(audioData.length);
        let mpg123AudioBufferSize;
        let mpg123AudioBufferDuration;
