...
export async function loadPackage(packageName) {
    ...
    await downloadAndExtractTarball(options, packagesPath, tempPath, packageName);
    return packagePath;
}
export async function removePackage(packageName) {
    packageName = resolveToVersionedPackageNameIfNeeded(packageName);
    const packagesPath = await ensureAndGetPackagesDir();
    const packagePath = path.join(packagesPath, packageName);
...
export function resolveToVersionedPackageNameIfNeeded(packageName) {
