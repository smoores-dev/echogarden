export function knuthMultiplicative(bytes) {
    let hash = 0;
...
export function xorShift32Hash(bytes) {
    let s = 0;
    for (const byte of bytes) {
        s += byte;
        s ^= s << 13;
...
export function jenkinsOneAtATime(bytes) {
    let hash = 0;
    for (const byte of bytes) {
        hash += byte;
        hash += hash << 10;
...
export function superFastHash(bytes) {
...
export function cyrb53Hash(bytes, seed = 0) {
...
export function murmurHash1(bytes, seed = 0) {
